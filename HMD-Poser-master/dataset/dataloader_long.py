...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'HMD_2IMUs' in self.compatible_inputs:
            input_feat = motion_data['input_feat'][idx : idx + self.input_motion_length].clone()
            input_feat[:, list(range(24, 30)) + list(range(54, 60)) + list(range(78, 81))] = 0  # 去根，补0
            input_feat_all.append(input_feat)
        if 'HMD' in self.compatible_inputs:
            input_feat = motion_data['input_feat'][idx : idx + self.input_motion_length].clone()
            input_feat[:, list(range(12, 30)) + list(range(42, 60)) + list(range(72, 81))] = 0  # 去根和双小腿，补0
            input_feat_all.append(input_feat)
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'HMD_2IMUs' in self.compatible_inputs:
            input_feat = motion_data['input_feat'].clone()
            input_feat[:, list(range(24, 30)) + list(range(54, 60)) + list(range(78, 81))] = 0
            input_feat_all.append(input_feat)
        if 'HMD' in self.compatible_inputs:
            input_feat = motion_data['input_feat'].clone()
            input_feat[:, list(range(12, 30)) + list(range(42, 60)) + list(range(72, 81))] = 0
            input_feat_all.append(input_feat)
