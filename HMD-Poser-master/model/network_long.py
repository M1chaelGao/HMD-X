...
class TemporalSpatialBackbone(torch.nn.Module):
    def __init__(self, input_dim, number_layer=3, hidden_size=256, dropout=0.05, nhead=8, block_num=2):
        super().__init__()
        assert input_dim == 81, "invalid input dim"
        self.head1_linear_embeddings = nn.ModuleList([nn.Sequential(nn.Linear(6, hidden_size//4), nn.LeakyReLU()),
        ...
        self.time_encoder = nn.ModuleList(
            ...
            bidirectional=False, batch_first=True) for _ in range(5)]) for _ in range(self.num_block)]
        )
    ...
    def forward(self, x_in, rnn_state=None):
        ...
        head_feats1 = [x_in[..., 0:6], x_in[..., 30:36], x_in[..., 60:63], x_in[..., 66:69]]    # 提取特征？？feature embedding 18
        ...
        lfoot_feats = [x_in[..., 12:18], x_in[..., 42:48], x_in[..., 72:75]]
        rfoot_feats = [x_in[..., 18:24], x_in[..., 48:54], x_in[..., 75:78]]
        pelvis_feats = [x_in[..., 24:30], x_in[..., 54:60], x_in[..., 78:81]]
        ...
        collect_feats = torch.stack([head1_emb,head2_emb, lfoot_emb, rfoot_emb, pelvis_emb], dim=-2).reshape(batch_size, time_seq, 5, -1)    # （768,40,8,256）！！！！ 8个部分
        
        # temporal and spatial backbone
        for idx in range(self.num_block):
            collect_feats_temporal = []
            for idx_num in range(5):
            ...
            collect_feats = self.spatial_encoder[idx](collect_feats_temporal.reshape(batch_size*time_seq, 5, -1)).reshape(batch_size, time_seq, 5, -1)
