...
def generate_input_features(bdata_poses, body_pose_world, bm):
    ...
    input_rotation_global_6d = rotation_global_6d[1:, [15, 4, 5, 0], :]
    ...
    hands_rotation_mat_in_head_space = rotation_global_matrot[      # 提取第15个关节，乘以胳膊的global关节
    ...
    rotation_velocity_handsinheadspace_r6d = utils_transform.matrot2sixd( # 手部角速度，在头部坐标系下
        rotation_velocity_handsinheadspace.reshape(-1, 3, 3)
    ).reshape(rotation_velocity_handsinheadspace.shape[0], -1, 6)

    hands_position_in_head_space = (position_global_full_gt_world[:, [20, 21], :] - position_global_full_gt_world[:, 15:16, :]).double().bmm(rotation_global_matrot[:, 15]) # 头部坐标系下手部位置
    
    foot_accs = _syn_acc(body_pose_world.v[:, [1176, 4662, 3021]])[1:, :, :]   # 1176：左小腿，4662：右小腿，3021：根
    ear_accs = _syn_acc(body_pose_world.v[:, [4071,516]])[1:, :, :]
    hmd_position_global_full_gt_list = torch.cat(
        [
            input_rotation_global_6d.reshape(num_frames, -1), # (num,4个点,6旋转)->(num,36)
            input_rotation_velocity_global_6d.reshape(num_frames, -1), # （num,4个点，6d旋转速度）-》（num,36）
            # position_global_full_gt_world[1:, [12,15], :].reshape( # (num,hmd3个点头左右手，3维位置)-》（num，9）
            #     num_frames, -1
            # ),
            # position_global_full_gt_world[1:, [12,15], :].reshape( # 同上点速度-》（num,9）
            #     num_frames, -1
            # )
            # - position_global_full_gt_world[:-1, [12,15], :].reshape(
            #     num_frames, -1
            # ),
            # hands_rotation_in_head_space_r6d.reshape(num_frames, -1), # 手臂相对于头部的旋转6d （num,2,6） ->(num,12)
            # rotation_velocity_handsinheadspace_r6d.reshape(num_frames, -1), # 手臂相对于头部的旋转速度6d （num,2,6） ->(num,12)
            # hands_position_in_head_space[1:].reshape(num_frames, -1), #手臂相对于头部的位置2个点，3维 （num,2,3） ->(num,6)
            # hands_position_in_head_space[1:].reshape(num_frames, -1) - hands_position_in_head_space[:-1].reshape(num_frames, -1), #速度同上，(num,2,3） ->(num,6)


            #这是原来的
            # foot_accs.reshape(num_frames, -1), # 两个足部（小腿）加上根结点3个点的加速度（num,3个点,3维旋转？）-》（num,9）
            ear_accs.reshape(num_frames, -1),
...
def process_protocol1(args, bm_male, bm_female):
    for dataroot_subset in ["MPI_HDM05"]:
        ...
        for phase in ["test"]:
            ...
            split_file = os.path.join(
                "/home/wh0146/HMD-Poser-master/prepare_data/data_split_tset", dataroot_subset, phase + "_split.txt"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--support_dir",
        type=str,
        default="/home/wh0146/HMD-Poser-master/body_models/",
        help="=dir where you put your smplh and dmpls dirs",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="/home/4T-2/wh/dataset/test_vis/hmd/",
        help="=dir where you want to save your generated data",
    )
    parser.add_argument(
        "--root_dir", 
        type=str, 
        default="/home/4T-2/wh/AMASS/data/",
